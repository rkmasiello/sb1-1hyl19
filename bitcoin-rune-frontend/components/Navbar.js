...
export default function Navbar() {
  ...
  const handleXverseConnect = async () => {
    try {
      // Simplified XVerse connection logic
    ...
    <nav className="bg-black bg-opacity-80 p-4 text-[#FD4C00] text-xl">
      <div className="container mx-auto flex justify-between items-center">
        <div className="flex space-x-4">
          <Link href="/register" className={`hover:text-[#FF6E2E] ${pathname === '/register' ? 'font-bold' : ''}`}>
            Register
          </Link>
          <Link href="/dashboard" className={`hover:text-[#FF6E2E] ${pathname === '/dashboard' ? 'font-bold' : ''}`}>
            Dashboard
          </Link>
        </div>
